                    ...
                    <ul class="list-group">
                      ...
                      <li class="list-group-item">Jenis Kelamin : <?= $jk?></li>
                            ...
                            <div class="text-center">
                                <form id="insertData">    
                                    <input type="hidden" id="hewanID" name="hewanID" value="<?= $dataHewan->hewanID ?>">
                                </form>
	...
	<script type="text/javascript">
		...
		$("#btn-publish").click(function(e){
            e.preventDefault();
            $.ajax({
                url : "<?= site_url('verifikasi/publish_hewan')?>",
                data : new FormData($("#insertData")[0]),
                type: 'POST',
                contentType: false,
                processData: false,
                success : function(e){
                    swal("Hewan Berhasil Publish","","success");
                    window.location = "<?=site_url('verifikasi/hewan')?>";
                }
            });
        });
        ...
        $("#btn-tolak").click(function(e){
            e.preventDefault();
            $.ajax({
                url : "<?= site_url('verifikasi/tolak_hewan')?>",
                data : new FormData($("#insertData")[0]),
                type: 'POST',
                contentType: false,
                processData: false,
                success : function(e){
                    swal("Hewan ditolak","","success");
                    window.location = "<?=site_url('verifikasi/hewan')?>";
                }
            });
        });
