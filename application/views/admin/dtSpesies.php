                            ...
                            <div class="card-body">
                                <form id="insertData">
                                    <div class="form-body">
                                        ...
                                        <hr>
                                        <input type="hidden" id="spesiesID" name="spesiesID" class="form-control" value="">
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                <label class="control-label">Spesies</label>
                                                <input type="text" id="spesiesNama" name="spesiesNama" class="form-control form-control-sm" placeholder="">
                                                </div>
                                        </div>
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                <label class="control-label">Keterangan Spesies</label>
                                                <textarea id="spesiesKeterangan" name="spesiesKeterangan" class="form-control" placeholder=""></textarea>
                                                </div>
                                        </div>
                                    </div>
      ...
      <script src="<?= site_url('assets/ckeditor')?>/ckeditor.js"></script>
		...
		$('textarea').each(function(){
	 	...
	 	$("#demo-foo-addrow").DataTable({
			...
			serverSide: true,
			ajax: {
			  url: "<?php echo site_url('Master/gDataTableSpesies') ?>",
			  type:'POST'
			}
		});
		$(document).on("click",".hapus",function(e){
			...
			function(){
				$.ajax({
					url : "<?= site_url('Master/deleteSpesies')?>",
					data : {spesiesID:id},
					type: 'POST',
					success : function(e){
		...
		$("#btn-update").click(function(e){
			e.preventDefault();
			$.ajax({
				url : "<?= site_url('Master/updateSpesies')?>",
		...
		$(document).on("click",".edit",function(e){
    		...
    		console.log(id);
    		$.ajax({
				url : "<?= site_url('Master/getIDSpesies')?>",
				data : {spesiesID:id},
				type: 'POST',
				success : function(e){
					var Obj = JSON.parse(e);
					$("#spesiesID").val(Obj.spesiesID);
					$("#spesiesNama").val(Obj.spesiesNama);
					$("#spesiesKeterangan").val(Obj.spesiesKeterangan);
					$("#btn-update").show();
					$("#btn-cancel").show();
					$("#btn-submit").hide();
				}
		...
		$("#insertData").submit(function(e){
			e.preventDefault();
			$.ajax({
				url : "<?= site_url('Master/insertSpesies')?>",
