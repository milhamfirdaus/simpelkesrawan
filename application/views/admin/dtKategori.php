                            ...
                            <div class="card-body">
                                <form id="insertData">
                                    <div class="form-body">
                                        ...
                                        <hr>
                                        <input type="hidden" id="kategoriID" name="kategoriID" class="form-control" value="">
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                <label class="control-label">Kategori</label>
                                                <input type="text" id="kategoriNama" name="kategoriNama" class="form-control form-control-sm" placeholder="">
                                                </div>
                                        </div>
                                    </div>
      ...
      <script src="<?= site_url('assets/ckeditor')?>/ckeditor.js"></script>
		...
		$('textarea').each(function(){
	 	...
	 	$("#demo-foo-addrow").DataTable({
			...
			serverSide: true,
			ajax: {
			  url: "<?php echo site_url('Post/gDataTableKategori') ?>",
			  type:'POST'
			}
		});
		$(document).on("click",".hapus",function(e){
			...
			function(){
				$.ajax({
					url : "<?= site_url('Post/deleteKategori')?>",
					data : {id},
					type: 'POST',
					success : function(e){
		...
		$("#btn-update").click(function(e){
			e.preventDefault();
			$.ajax({
				url : "<?= site_url('Post/updateKategori')?>",
		...
		$(document).on("click",".edit",function(e){
    		...
    		var id = $(this).attr("data-id");
    		$.ajax({
				url : "<?= site_url('Post/getKategoriByID')?>",
				data : {id:id},
				type: 'POST',
				success : function(e){
					var Obj = JSON.parse(e);
					$("#kategoriID").val(Obj[0].kategoriID);
					$("#kategoriNama").val(Obj[0].kategoriNama);
					$("#btn-update").show();
					$("#btn-cancel").show();
					$("#btn-submit").hide();
				}
		...
		$("#insertData").submit(function(e){
			e.preventDefault();
			$.ajax({
				url : "<?= site_url('Post/insertKategori')?>",
